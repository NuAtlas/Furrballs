...
The central finding is that **integer quantization is load-bearing, not a compression technique** (Finding 2). Float64 EMA allows R → 0.99999 for hot keys, making (1−R) ≈ 10⁻⁵. After 10 decay steps the state jumps 1000×, causing hypersensitivity and cascading evictions. Integer truncation floors (MAX−R) ≥ 1, preventing this divergence. The 4-bit and 8-bit integer variants produce nearly identical results, confirming that B controls effective dynamics (α/2^B) rather than precision.
...
**Finding 14 (ARC Dominates Single-Node Zipfian).** ARC achieves 44.7% Zipfian hit rate — nearly 2× REMARC's ceiling. This is not a deficiency of REMARC; REMARC is designed for NUMA multi-node decisions where per-key scoring provides structural advantages (desire encoding, §5.3). Single-node Zipfian is the wrong benchmark for REMARC.
