...
This is the paper's most concrete theoretical result. It gives a **deterministic relationship between observed access frequency and REMARC state**: the state vector encodes the empirical access rate distribution across sources. No stationarity or independence assumptions required — $p_j$ is an observable quantity. Design implications:
...
**Why diagonal matters:** for general (non-diagonal) matrices, the JSR can exceed individual spectral radii because off-diagonal coupling in products can amplify eigenvalues. Diagonal matrices have no off-diagonal coupling. Each entry of the product $M_{\sigma(t)} \cdots M_{\sigma(1)}$ is a product of scalars, each strictly less than 1, decaying independently. No amplification is possible.

**Convergence behavior by access pattern:**

| Switching sequence | Convergence |
|---|---|
| Each source has limiting frequency $p_j$ | Converges to steady-state formula $S_j^*$ |
| Periodic (e.g., alternating sources) | Converges to periodic orbit |
| Adversarial (worst case) | State stays bounded, homogeneous transient decays to 0 |
...
|---|---|---|---|
| **LRU** (recency) | $S_0$ = EMA of recency | K=1, M=1, $A = -1$ | Exact |
| **LFU** (frequency) | $S_0$ = accumulating EMA of frequency | K=1, M=1, $A = -1$ | Exact |
| **Clock** (reference bit) | $S_0$ = binary EMA (α→1, λ→0) | K=1, M=1 | Approximate |
...
| **LIRS** (reuse distance) | IRR ≈ EMA recency | K=1, M=1 | Approximate |
| **ARC** (ghost lists) | Ghost membership ≈ binary EMA | K=2, M=1 | Approximate |

**"Exact"** means the REMARC configuration produces the same decision on every access sequence. **"Approximate"** means the decisions converge as α and λ are tuned to approach the policy's behavior.
