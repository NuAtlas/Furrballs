...
We evaluate REMARC through a comprehensive simulation study across three regimes: single-node (20 variants, 4 workloads), 2-node NUMA (10 experiments, 4 workloads), 3+ node NUMA (3 variants × 4 workloads with parameter sweep), and augmentation (Section 8, 8 findings). All experiments use integer quantized state (uint8 per axis) with the REMARC update rules from §3.2.
...
We tested 20 cache policies: LRU, ARC, 17 REMARC variants (covering bare REMARC, lazy increment, S3-FIFO admission, LFU, TinyLFU, ghost field, ghost gate, bloom gate, bloom modulate, factorized, tiered ghost, dual, multi-scale, step, step-K, augmented timestamp, and augmented predictive), all under identical conditions.
...
**Finding 1 (Locality Barrier).** No REMARC variant within the integer EMA per-key scoring class exceeds ~24.5% Zipfian hit rate at 10% capacity. This ceiling is robust across 15 page-batch variants including lazy increment, S3-FIFO admission, LFU, TinyLFU, ghost field, bloom modulate, factorized, tiered ghost, multi-scale, and step. The page-batch ceiling is structural: page-level eviction requires evicting all keys in a page, discarding the per-key scoring signal. Only variants that break page-batch structure (AUG-TS4 at 33.93%, AUG-PRED at 33.92%) approach ARC's 35.17%.

**Finding 2 (Integer Quantization Is Critical).** The page-batch variants with integer quantized state maintain 90%+ ScanRes (REMARC-OPT: 65.7%, REMARC-LFU: 65.7%), while the non-quantized variants (bare REMARC, REMARC-LazyInc) collapse to 53% on Temporal Shift. Integer truncation floors (MAX−R) ≥ 1, preventing floating-point hypersensitivity.
...
**Finding 6 (Tiered/Capacity Allocation Is Neutral).** REMARC-TIER (24.45% Zipf) matches REMARC-OPT exactly. Unlike the RemarcBench experiments where p collapsed to pMin, PolicyBench's per-key REMARC state provides sufficient discrimination that capacity splitting adds no value.
...
#### 5.2.3 Notable Variant Behaviors
...
#### 5.2.4 Locality Barrier Analysis
...
Note: This study uses the RemarcBench framework (ops=200k, page-batch delivery) which differs from the PolicyBench framework (ops=300k, per-key delivery) used in §5.2. The ghost map data here is from the original REMARC implementation.
...
| 14 | ARC dominates single-node Zipfian (2×) | REMARC targets multi-node; single-node not primary metric |
...
#### 8.3.1 Ghost List Bug Discovery
...
#### 8.3.3 Lazy Decay and the Frozen State Problem
...
#### 8.3.4 Timestamp-Based Decay (AUG-TS)
...
#### 8.3.5 Predictive Eviction (AUG-PRED)
...
**Finding 29 (Throughput--Accuracy Tradeoff).** AUG-HEAP achieves 6.14M ops/s on Zipfian (vs ARC's 5.44M), demonstrating that heap-based eviction can match ARC's throughput despite O(log n) eviction cost. The amortized cost is low because eviction occurs only on cache misses (not hits), and the 64-entry scan limit bounds worst-case cost.

### Adaptive Delivery: AUG-ADAPT

The delivery--quota tension (Section 9.6) motivates a **self-tuning** variant that adapts its delivery mechanism to the workload. AUG-ADAPT combines two signals to choose between heap-based and LRU-based eviction at runtime:
...
The evaluation (Sections 5, 8, 9) establishes five principal results across single-node (23 variants, 4 workloads), 2-node (10 experiments, 4 workloads), 3-node (3 variants, 4 workloads), augmentation (8 findings), and structured eviction (5 findings) configurations:

1. **Single-node ceiling ($\sim$24.5% Zipfian at 10% capacity):** No tested REMARC variant within the bounded per-key scoring class exceeds this ceiling, which we attribute to ARC's T1/T2 structural identity providing more information per capacity unit than scalar scoring can express. This holds across all tested variants including feedback modulation, capacity allocation, and ghost restoration. REMARC compensates with structural dominance on looping workloads (49% vs 0% for ARC/LRU) and strong scan resistance ($\sim$90%).

2. **Augmentation produces marginal gains over ARC (Section 8):** Enriching ARC with REMARC's state space ($S = (\text{recency}, \text{frequency}, \text{period}, \text{confidence})$) and evidence-based demotion produces consistent but modest improvements: +1.59pp ScanRes, +0.59pp Temporal, at a cost of -1.24pp Zipfian. The augmentation is a strict superset of ARC ($\theta_{demote} = 2$ reproduces ARC exactly). More significantly, the investigation revealed a **structural barrier**: the list delivery mechanism prevents acting on correct predictions for adversarial (looping) workloads (Finding 25), motivating future work on structured eviction.
...
The representation framework (§3.4) and factorization analysis (§9.1) suggest that REMARC captures an algebraic structure common to the class of history-based online sequential decision functions. This section outlines the theoretical direction; full formalization is planned as Paper 3.
