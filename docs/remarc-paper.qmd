...
6. A companion systems paper validates REMARC's NUMA placement signal on real hardware, confirming the architectural motivation for the migration axis.
...
### Existing Cache Policies Are One-Dimensional
...
We tested 24 cache policies: LRU, ARC, 16 page-batch REMARC variants, 4 augmented ARC variants (AUG-TS4, AUG-PRED, AUG-HEAP, AUG-BOTHEND), AUG-ADAPT (self-tuning), and QuotaFree (pure predictive, no ARC structure), all under identical conditions.
...
This section describes the practical architecture for deploying desire-encoded REMARC in a NUMA-aware cache. The design targets a shared-memory NUMA system with per-node page allocation and lazy migration.
...
This appendix documents the reference implementation architecture for desire-encoded REMARC in a NUMA-aware cache.
...
The generalization in §6 shows that REMARC's policy framework is independent of any specific cache infrastructure. Any cache system with per-item state storage can adopt REMARC. The desire encoding mechanism requires only:
