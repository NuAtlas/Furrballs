...
3. SIMD-optimized implementation using precomputed lookup tables (256 entries, SSE2 direct arithmetic, O(1) amortized per 16 keys).
...
- **SIMD throughput:** batch scan rate (SSE2 direct arithmetic) vs sequential scan vs CMap probe
...
**4. Learnability (depends on Q1, Q2).** Can the REMARC parameters $(\alpha, \lambda, A_m)$ be learned from workload traces? Is there an online learning algorithm that converges to the optimal REMARC point for a given workload?
