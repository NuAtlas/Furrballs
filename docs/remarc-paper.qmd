...
**Discrimination space D.** A compact ordered subset of R^m, m > 0.
- Bounded because decisions are definite values (there is no semantically meaningful "infinity-strength decision")
- Ordered because decisions must be comparable (higher score = stronger evidence for that action)
- Boundedness is independent of S — it follows from the semantics of what a decision IS, not from the function that produces it

**Key property:** S and D are bounded independently. No circularity. S is bounded because compression requires it. D is bounded because decisions are definite values. The function f: S -> D bridges two independently bounded spaces.
...
The K=1 REMARC function is the **atom** of the decision function space. The atom is: "track one exponentially-smoothed signal, decay it over time." Compose enough atoms with different α and λ values, project their outputs, and you get any sequential decision function in the space.
