...
#### 5.2.2 Quantization Comparison

Figure 1 isolates the effect of numeric representation across the four core workloads. The data covers four quantization classes: **Float64** (factored EMA with R,F ∈ [0,1]), **4-bit integer** (bare REMARC), **8-bit integer** (factored FI-Mult), and **Log8 integer** (logarithmic quantization, best-tuned).

![Figure 1: Hit rate by workload across quantization classes. Integer quantization preserves ScanResistant (~90%) and enables Looping (~49%). Float64 collapses ScanResistant to 48% and scores 0% on Looping. Log8 trades Looping for a marginal Zipfian gain.](fig-quantization.svg){#fig-quantization fig-align="center"}

The central finding is that **integer quantization is critical for scan resistance** (Finding 2). Float64 EMA allows R → 0.99999 for hot keys, making (1−R) ≈ 10⁻⁵. After 10 decay steps the state jumps 1000×, causing hypersensitivity and cascading evictions. Integer truncation floors (MAX−R) ≥ 1, preventing this divergence. The 4-bit and 8-bit integer variants produce nearly identical results, suggesting that B controls effective dynamics (α/2^B) rather than precision.

#### 5.2.3 Key Findings (Single-Node)

**Finding 1 (Locality Barrier).** No REMARC variant within the integer EMA per-key scoring class exceeds ~24.5% Zipfian hit rate (cache-only, 10% cap/keys ratio). Ghost restoration adds at most +1.5pp. This ceiling is robust across all tested D_f (scalar scoring, Q-modulated scoring, directional scoring, capacity allocation, logarithmic quantization). ARC achieves 44.7% through structural identity — its T1/T2 partition provides population-level information that per-key scalar scoring cannot express.

**Finding 2 (Integer Quantization Is Critical).** Replacing integer EMA with floating-point EMA collapses ScanRes hit rate from 90.4% to 48.2% across all tested formula variants. The discretization acts as a noise filter on the frequency axis, preventing scan pollution from inflating sF. We conjecture this is a structural requirement rather than a tuning artifact, but a formal proof is open.
...
**Finding 5 (Feedback Is Signal-Degenerate in Current Design).** Q-modulated proj_s weights and directional (Q_r, Q_f) feedback provided no measurable improvement over static proj_s across all tested configurations. Cause: the REMARC state update phi produces uniform population statistics (G, H, Q) because all keys receive identical boost/decay treatment. Without discriminable sub-populations, feedback has no gradient to exploit. Whether non-uniform phi (e.g., per-key adaptive rates) would enable effective feedback is an open question.

**Finding 6 (Tiered/Capacity Allocation Collapsed in Our Tests).** ARC-like T_R/T_F partition with p-controlled split caused p to collapse to pMin in all tested configurations. Ghost_R filled with cold revolving-door keys — recently evicted items from T_F re-enter T_R at zero cost, preventing meaningful capacity rebalancing. This may be specific to the p-update rule we used; alternative mechanisms (e.g., frequency-gated admission) are not excluded.

#### 5.2.4 Locality Barrier Analysis

The ~24.5% Zipfian ceiling is an empirically observed property of the per-key scalar scoring class at 10% cap/keys ratio, not of REMARC specifically. Any policy where D_evict = g(S(K)) for scalar S(K) produces a total order on keys. ARC's T1/T2 partition breaks this total order — it maintains two populations with distinct admission criteria, providing strictly more information per capacity unit.

An informal argument: for a Zipf(α) distribution with α → 1, the optimal fixed-threshold scalar policy captures the top-K keys by score. ARC's adaptive partition captures a different subset — recent-but-infrequent keys in T1 that would be evicted by any scalar policy but contribute to future hits via ghost-mediated promotion. A formal proof that scalar scoring cannot exceed a specific bound would require characterizing the optimal scalar policy's hit rate as a function of α and cap/keys ratio; we leave this open.
...
**Finding 14 (ARC Dominates Single-Node Zipfian).** ARC achieves 44.7% Zipfian hit rate — nearly 2× REMARC's best observed single-node performance. This reflects ARC's structural advantage (T1/T2 partition) for single-node eviction. REMARC's design target is multi-node decisions where per-key scoring provides structural advantages (desire encoding, §5.3); single-node Zipfian is not the primary evaluation metric for this framework.
...
Based on the simulation results (§5.3, §5.4), the benefits projected for Furrballs on a 2-socket c6i.metal (subject to real-hardware validation):
