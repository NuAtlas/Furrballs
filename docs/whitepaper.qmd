...
subtitle: "Technical Report v1.11.0"
...
## Status
v1.11.0 --- REMARC policy framework paper (v0.7.0, DOI: 10.5281/zenodo.19794758) complete with 23 variants, AUG-ADAPT self-tuning feedback controller, 30 findings. PolicyBench simulation harness. Whitepaper updated with REMARC citation, engineering decisions, variant taxonomy.
...
**Variant taxonomy.** 23 policy variants were evaluated in PolicyBench, classified by projection mechanism:
...
**Policy as a compile-time type parameter.** Furrballs uses `FurrBall<Policy>` where Policy is a template type (§2.8). This was extended to support REMARC variants during PolicyBench development. All 23 variants share a common `access(uint64_t key)` interface with `hits()`, `misses()`, `evictions()`, and `scans()` queries. No virtual dispatch, no runtime polymorphism. This design validated cleanly --- adding variants required only template instantiation, no interface changes.
...
- [x] Policy framework validation: 23 variants evaluated in PolicyBench, compile-time template type parameter
