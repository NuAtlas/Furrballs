...
subtitle: "Technical Report v1.7.3"
...
## Status
v1.7.3 --- Wide pass review: update stale forward references (§2.1.2, §2.4), align methodology with real hardware iterations (§5.1), mark completed items (§4.3 scaling, §5.9 validation), note Phase 2a resolution in tradeoff model (§5.12)
...
Multi-value pages use an append-only bump allocator with 8-byte alignment. Each page tracks `UsedBytes` (atomic, CAS-based concurrent bump) and `DataWastedByPadding` for fragmentation metrics. No per-value deallocation --- pages are evicted wholesale by ARC. Phase 2a's ConcurrentARC tracks key-level hot/cold state and provides eviction callbacks; Phase 2b will wire these callbacks to flush dirty values to RocksDB and reclaim page memory.
