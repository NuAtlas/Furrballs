...
subtitle: "Technical Report v1.9.1"
...
Furrballs applies the scores with mutual exclusion: `if E > θ_e: evict` then `elif M > θ_m: migrate to HotNode`. A cold key (high E) is evicted, not migrated. A misplaced hot key (high M, low E) is migrated, not evicted.
...
```
On local Get():
  S_local  += α_l × (15 - S_local)        // boost local signal toward max
  S_remote *= (1 - α_r)                     // erode remote signal (recency anti-migration)

On remote Get() from node N:
  S_remote += α_r × (15 - S_remote)        // boost remote signal toward max
  S_local  *= (1 - α_l)                     // erode local signal (migration pressure builds)
...
Phase 2b will wire the eviction callback to flush dirty values to RocksDB, completing the L1 key-level ARC. The broadcast-race dispatch model (Phase 4) provides O(N) aggregate work with O(1)-round parallel latency.
