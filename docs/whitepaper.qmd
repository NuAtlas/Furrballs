...
subtitle: "Technical Report v1.1"
author: "The Sphynx"
date: last-modified
lang: en
format:
...
The broadcast-race Get replaces the O(N) sequential shard scan with a parallel scatter-gather: all N node workers probe their local shard concurrently, achieving wall-clock latency of a single shard lookup (independent of node count) while total CPU work remains O(N) unsuccessful probes.
...
- [ ] Score-based eviction policy with configurable weight vector (recency, frequency, cross-node access count). Enables empirical testing of eviction behavior across a range of weight combinations rather than committing to a single fixed policy like ARC.
...
Phase 2 will introduce the novel contribution: a two-level ARC hierarchy where NUMA placement and cross-node access frequency serve as eviction policy inputs. The broadcast-race dispatch model (Phase 4) provides a path to O(1) key location resolution using structural single-writer guarantees and WaitGroup coordination, with the caller fully decoupled from the data path.
