...
subtitle: "Technical Report v1.30.0"
...
## Status
**v1.30.0** --- Fixed ReadOnly key prefix bug (pre-populate used `"key_"`, workload used `"p_"`: all previous ReadOnly data measured empty CMap probes at 43ns, not data reads). All ReadOnly and UniformRO benchmarks re-run on c6i.metal. CacheLib updated from v2024.07 to v2026.02 (no meaningful performance difference, <4% delta). NUMA DRAM penalty measured via UniformRO workload at 64MB and 128MB: 171ns and 211ns per-node penalty. CacheLib's Memory Tiers feature (NUMA-aware memory placement) is not implemented in any open-source release (v2024 or v2026) --- the config API exists but the allocator throws at runtime with `// TODO: we support single tier so far`. FurrBallSN achieves 10.9x (64MB) and 13.9x (128MB) lower p50 latency than CacheLib at equal usable capacity.
