...
subtitle: "Technical Report v1.29.0"
...
## Status
**v1.29.0** --- Corrected-capacity NUMA comparison at 1024B values. All systems now at exactly 32MB usable capacity. Critical finding: Remote-Only adapter shows 43ns identical to SN, proving 32MB working set fits in L3 cache. The TL-vs-CN gap (56ns vs 191ns) is cache locality (cross-socket L3 metadata traffic), not NUMA DRAM locality. NUMA DRAM penalty not yet measured --- requires working-set sweep beyond L3 capacity (56MB). Memory efficiency: FurrBall 100% (32MB/32MB) vs CacheLib 50% (64MB/32MB). Remote-Only data confirms no remote DRAM penalty at this working set size.
...
> **Status: Corrected capacity.** All systems at 32MB usable. Previous runs had unequal capacities (FurrBall 32MB, CacheLib ~16MB usable, CacheLibNuma 64MB due to floor-bump bug). Capacity alignment and footprint tracking added in commit `d6c66c4`. Remote-Only adapter added in `bfc937b`, data collected. Critical finding: the 32MB working set fits in the shared L3 cache (56 MB on Ice Lake), so the TL-vs-CN latency difference is driven by **cache locality and cache line traffic**, not NUMA DRAM latency. See NUMA Penalty Decomposition below.
...
#### Results: ReadOnly 2t, 1024B, 32MB (No Eviction)

| System | p50 Get | p99 Get | ops/s | Footprint | node0 | node1 |
|--------|---:|---:|---:|---:|---:|---:|
| FurrBall LRUSN | 43 ns | 141 ns | 0.81M | 32 MB | 43 | - |
| FurrBall SN | 43 ns | 139 ns | 0.81M | 32 MB | 43 | - |
| FurrBall LRUCN | 55 ns | 218 ns | 0.95M | 32 MB | 56 | 54 |
| FurrBall TL | 56 ns | 298 ns | 2.17M | 32 MB | 57 | 56 |
| FurrBall LRUTL | 59 ns | 259 ns | 2.19M | 32 MB | 60 | 59 |
| CacheLibNuma | 126 ns | 656 ns | 2.27M | 64 MB | - | - |
| CacheLib | 156 ns | 662 ns | 1.19M | 64 MB | - | - |
| FurrBall CN | 191 ns | 314 ns | 0.93M | 32 MB | 191 | 191 |
| FurrBall Remote | 43 ns | 202 ns | 0.61M | 32 MB | 43 | 43 |
| FurrBall LRURemote | 43 ns | 196 ns | 0.62M | 32 MB | 43 | 43 |
...
The c6i.metal platform has a 56 MB shared L3 cache per socket (the NUMA distance matrix 10/20 means remote DRAM is ~2x the latency of local DRAM). At 32MB working set, all data fits in L3, and every read is an L3 hit regardless of physical NUMA placement. This has a critical implication for interpreting the benchmark results.

| Configuration | L3 Traffic Pattern | p50 Get (ns) | Source |
|---|---|---:|---|
| SN (single node) | 1 CMap's metadata + data | **43** | ReadOnly |
| Remote-Only (1-node data, thread on node 1) | 1 CMap's metadata + data, remote L3 | **43** | ReadOnly |
| TL (2 nodes, thread-local) | 1 CMap's metadata + data | **56** | ReadOnly |
| CN (2 nodes, RoundRobin) | 2 CMaps' metadata + data | **191** | ReadOnly |

**The 43ns baseline.** Both SN and Remote-Only achieve 43ns. This is the cost of: one CMap seqlock probe (~15ns) + 1KB memcpy from L3 (~25ns) + PromoteBuf enqueue (~3ns). The fact that Remote-Only = SN confirms NUMA DRAM is irrelevant at this size.

**The TL 56ns overhead.** TL adds 13ns over the 43ns baseline. TL has 2 CMaps (one per node), but each thread only accesses its own node's CMap. The 13ns comes from the per-node routing layer: hash-to-node mapping, node selection, and the two-node CMap structure's overhead (checking which node a key belongs to). This is the architectural cost of topology-awareness.

**The CN 191ns penalty.** CN distributes keys across both nodes. Every Get accesses whichever CMap holds the key --- statistically, 50% on each node. The per-node p50 shows node0=191ns, node1=191ns (identical, confirming the blend). The 135ns penalty (191 - 56) comes from **cross-socket L3 cache line traffic for CMap metadata**. When thread 0 accesses a key stored on node 1's CMap, the CMap's control byte array (seqlock metadata) is on node 1's L3 slice. This cache line must cross the UPI interconnect to reach thread 0's socket. The 1KB data itself may also cross UPI if the page was allocated on the other socket. With RoundRobin routing, ~50% of Gets cross the interconnect for CMap metadata, and the data memcpy may also be remote.

**What this means for the thesis.** At 32MB working set (fits in L3), topology-aware placement's benefit is **cache locality**, not DRAM locality. TL confines each thread's CMap metadata and data to one socket's L3 slice, eliminating cross-socket cache line transfers. This is a real and measurable benefit (3.4x), but it operates at the cache level, not the DRAM level.

**At what size does NUMA DRAM matter?** When the working set exceeds 56MB L3, cache misses force DRAM access. At that point, NUMA placement would matter directly. We have not measured this crossover. A working-set sweep (64MB, 128MB, 256MB) with 1024B values would identify the point where L3 capacity is exceeded and NUMA DRAM cost becomes dominant. This is a natural extension of the current results.

#### Memory Efficiency

| System | Usable Capacity | Memory Footprint | Efficiency |
|--------|---:|---:|---:|
| FurrBall | 32 MB | **32 MB** | **100%** |
| CacheLib | 32 MB | **64 MB** | 50% |
| CacheLibNuma | 32 MB | **64 MB** | 50% |
| TBB | unbounded | unbounded | N/A |

FurrBall achieves 100% memory efficiency (every allocated byte stores user data or essential metadata). CacheLib's slab allocator reserves half the configured memory for internal structures (slab headers, allocation class bitmaps, free lists). For the same 32MB usable capacity, CacheLib requires 64MB of physical memory --- a **2x memory tax**.
...
3. **The throughput gap is eviction, not architecture.** FurrBall's p99_set (35-67us) vs CacheLib's (3.7-4.7us) explains the entire ops/s gap. Background eviction via NodeJob workers would close this.
