...
## Status
**v1.26.0** --- PRELIMINARY multi-baseline NUMA comparison. NUMABench run on c6i.metal with 4 systems (FurrBall TL, FurrBall RR, TBB, CacheLib) across 3 workloads (partitioned, shared, trace replay). FurrBall TL is 15% faster than CacheLib on local gets (211ns vs 248ns) despite 32× less capacity, single SpinLock per node, and doing real ARC eviction. Results are preliminary (single run, no capacity normalization, no confidence intervals). Production path: ARC eviction + SimpleMigratePolicy. Migration (the second half of the value proposition) remains untested.
...
1. **32× less capacity.** FurrBall at 64 pages/page = 512 KB total. CacheLib's 32 MB minimum means it barely evicts under our trace workload. TBB has no capacity limit at all. This directly affects hit rate on trace replay (the only workload where capacity matters).
...
- **Trace replay (Twitter cluster 52, 1M entries):** Full trace replay with realistic access patterns. Hit rate reflects both policy quality and effective capacity. This is where the capacity handicap dominates.

#### Results: Partitioned (2 threads, 64 pages)

| System | p50 Get (ns) | p99 Get (ns) | p50 Set (ns) | Hit Rate |
|--------|---:|---:|---:|---:|
| FurrBall TL | 211 | 491 | 426 | 95.7% |
| FurrBall RR | 462 | 1,101 | 1,716 | 95.7% |
| TBB | 58 | 568 | 292 | 95.7% |
| CacheLib | 248 | 890 | 1,264 | 95.7% |

The NUMA signal is isolated by comparing FurrBall TL vs RR (same system, same capacity, only routing differs): **211ns vs 462ns = +251ns (+119%)** overhead for cross-NUMA key placement. This is the genuine cost of putting a key on the wrong NUMA socket, compounded by double-lookup penalties (try local node first, miss, try remote) and multiple remote cache-line misses per operation.

FurrBall TL is **15% faster than CacheLib on local gets** (211ns vs 248ns) despite having 32× less capacity and doing real ARC eviction while CacheLib barely evicts. CacheLib's per-bucket striped locking provides no advantage at 1 thread per node.
...
|--------|---:|---:|---:|
| FurrBall TL | 635 | 1,263 | 95.7% |
| FurrBall RR | 615 | 1,144 | 95.7% |
| TBB | 612 | 871 | 95.7% |
...
1. **Partitioned** measures the raw NUMA topology signal. FurrBall TL's 211ns vs RR's 462ns is a clean +119% overhead for cross-NUMA access, validating the thread-local routing thesis. FurrBall TL's 15% get-speed advantage over CacheLib (211ns vs 248ns) despite 32× less capacity is the key finding: FurrBall's page arena + FlatList ARC has lower per-operation overhead than CacheLib's slab allocator, and the gap is real.

2. **Shared** measures concurrency model quality. CacheLib's striped locking (439ns) beats FurrBall's single SpinLock (635ns) by 31%. This identifies the concurrency model as the next bottleneck to address.

3. **Trace replay** measures the combined effect of policy quality and capacity. The 3.5pp hit rate gap between FurrBall (76.6%) and CacheLib/TBB (~80%) is almost entirely a capacity artifact. Normalizing capacity would either close this gap (validating ARC's superiority over LRU) or widen it (if ARC's overhead negates the hit rate benefit). This test cannot distinguish the two without capacity normalization.

#### Why These Results Are Directionally Informative But Not Publishable

The directional conclusions are:

- NUMA-aware placement provides a genuine, measurable benefit (+119% for cross-NUMA avoidance)
- FurrBall's access path is architecturally efficient (15% faster gets than CacheLib despite handicaps)
- The concurrency model is the binding constraint for shared workloads
- The capacity handicap obscures the policy comparison

What's missing for publication:

- **Capacity normalization.** Run CacheLib at 512KB total (same as FurrBall) to isolate policy quality from capacity.
- **Confidence intervals.** This is a single run. EC2 spot instances have ~3.8pp Cross-OH variance within a run (§5.18 ablation stddev), so cross-run variance could be significant.
