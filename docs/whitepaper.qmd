...
subtitle: "Technical Report v1.8.0"
...
## Status
v1.8.0 --- Phase 2b design: migration-based eviction (compactor, page lifecycle HOT/COLD/EMPTY/FREEZE, TempCtrl, whole-page persistence, cross-node migration)
...
`KeyIndex` and `TempCtrl` are parallel arrays: `TempCtrl[i]` corresponds to `KeyIndex[i]`. `TempCtrl` encodes each key's temperature in a byte (inspired by Swiss table ctrl bytes, §2.7), enabling SIMD-scannable cold key detection by the compactor (§2.9). The eviction callback writes under ConcurrentARC's SpinLock; the compactor reads without a lock (benign stale reads are acceptable).
...
**Eviction:** `replaceLocked(h2)` and `evictLocked()` return bool --- false on CAS contention, callers propagate as ABANDONED_SET. List mutation happens AFTER confirmed `FindAndEraseByHash`. EvictionCallback receives `const Value&` (KeyMeta contains all metadata: PageIndex, DataSize, DataOffset, NodeID).

**Phase 2b eviction callback:** When ARC evicts a key (t1/t2 → b1/b2), the callback sets `page.HasColdKeys = true` and updates `page.TempCtrl[i] = 0x01` (COLD) for the evicted key. This flags the page for the compactor (§2.9). The callback runs under ConcurrentARC's SpinLock, so `TempCtrl` writes are serialized per node. No I/O, no allocation, no blocking operation on the callback path.

**Phase 2b promotion callback:** When PromoteBuf drain promotes a key (b1/b2 → t1/t2), the corresponding `page.TempCtrl[i]` is set back to `0x00` (HOT). This prevents the compactor from unnecessarily migrating a re-promoted key. The promotion drain already holds the SpinLock, so the `TempCtrl` write is serialized.
...
```
compactor_run(node):
  for each page where page.HasColdKeys == true:
      ...
      for each bit i set in cold_mask:
        ...
        if keyMeta has_value:
          ...
          if coldPage found:
            ...
            if newOffset valid:
              memcpy(coldPage.Data + newOffset, page.Data + keyMeta.DataOffset, keyMeta.DataSize)
...
cross_node_migrate(hashPair, destNode):
  // Remove from source node's CMap
  FindAndEraseByHash(hashPair) on source node
  // Allocate space on destination node's HOT page
  ...
  // Insert into destination node's CMap
  CMap.Insert(hashPair, KeyMeta{destPage, newOffset, ...})
  keyMeta.CrossNodeAccesses = 0
  // Update page bookkeeping on both nodes
```

The CMap entry fully relocates to the destination node, preserving the sole-owner invariant (a key belongs to exactly one node at any time). Cross-node migration is rare (high threshold) and represents the thesis's NUMA-aware placement contribution: data moves to where the threads that access it are.
...
- [ ] Cross-node migration: `FindAndErase` source → `memcpy` → `Insert` dest (triggered by CrossNodeAccesses threshold)
