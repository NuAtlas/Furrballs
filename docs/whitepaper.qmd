...
subtitle: "Technical Report v1.20.0"
...
## Status
v1.20.0 --- §5.23 updated with HYBRID-POS (position+score) and HYBRID-FB (feedback) results. POS preserves recency but loses loop detection. FB never triggers. Cross-reference to REMARC paper Findings 32-34. REMARC paper v0.10.0 with three new findings.
v1.19.0 --- §5.23 ARC-REMARC Hybrid: negative finding. REMARC per-key TempCtrl scoring does not subsume ARC p-adaptation. Hybrid wins Scan-Res (+2%), Looping (+97%) but loses Zipfian (-5%), Temporal (-21%). Mechanisms are complementary, not substitutable. Supports REMARC positioning as multi-action framework (eviction+migration), not better eviction.
v1.18.0 --- Strategic reframe from "NUMA-aware cache" to "topology-aware resource placement for asymmetric memory systems." Title, abstract, problem statement, architecture, evaluation, related work, conclusion updated. NUMA framed as first validation instance, CXL/HBM tiers as target. ~90 mechanical substitutions across all sections.
...
**Phase 1 is complete.** **Phase 2a is complete.** **Phase 2b data collected but evaluation deferred** (§5.20: page-level eviction smothers policy differentiation). **ARC-FLATLL2 optimized** (uint32_t indices + arena + no cached_: +87--172% over ARC, ranked #2 KPI). **ARC-REMARC Hybrid tested** (§5.23: negative finding, REMARC scoring does not subsume p-adaptation, complementary not substitutable).
