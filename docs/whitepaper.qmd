...
subtitle: "Technical Report v1.8.1"
...
## Status
v1.8.1 --- Fix concurrency bugs in Phase 2b design: compactor data-loss race (memcpy inside UpdateInPlace), KeyIndex/TempCtrl concurrent modification (per-page CompactLock), SIMD overread mask, FREEZE page skip, transient miss window on cross-node migration, eviction callback signature change
...
**Transient miss window:** Between `FindAndEraseByHash` (source) and `CMap.Insert` (dest), the key exists in no CMap. Concurrent `Get()` calls during this window see a spurious cache miss. This window is ~200--500ns on real hardware (two CAS cycles + one memcpy). Since cross-node migration is triggered only when `CrossNodeAccesses` exceeds a high threshold (the key has been cross-accessed hundreds of times), the probability of a concurrent Get hitting this exact window is negligible. A future optimization could add a `MIGRATING` ctrl byte state as a forwarding tombstone (readers follow the forwarding pointer to the destination node), but this is not required for Phase 2b.
