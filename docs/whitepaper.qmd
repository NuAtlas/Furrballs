...
subtitle: "Technical Report v1.7"
...
## Status
v1.7 --- Real hardware validation on AWS c6i.metal (Intel Xeon Platinum 8375C, 2 sockets, 128 vCPUs): NUMA signal confirmed (4.5--7.0% p50 cross-node overhead), thread-local routing +60--65% improvement, Get throughput scales +72% from 4→64 threads
...
- **Find (Get):** Lock-free CMap read via seqlock protocol. PromoteBuf enqueue for deferred ARC promotion (lock-free `fetch_add`). No SpinLock on the read path.
- **Set (insert/update):** UpdateInPlace fast path (CMap seqlock only, no SpinLock) for existing keys that fit. New-key path: SpinLock for ARC list management + CMap for concurrent key→value storage. CMap's per-slot CAS eliminates shared_mutex serialization.
- **Eviction:** SpinLock + CMap's `FindAndEraseByHash` (hash-keyed, no string needed).
...
**Find(key):** `store_.Find()` (lock-free CMap read) + `promoteBuf_.enqueue(hash)` (lock-free). Cooperative `try_lock` drain every 64th call. Returns `std::optional<Value>`.

**Set(key, val):** SpinLock + drain buffer + single `CMap.Set()` probe + hash-keyed ARC list management (standard ARC algorithm using H2 for list tracking). Returns `CMapSetResult`.
...
The concurrent **Set ratio of 1.74x** is the fair comparison (both take locks on writes; Furrballs wins through lock partitioning). The baseline concurrent Get anomaly (18M ops/s from unsynchronized `unordered_map::find()`) is the same data race documented in §5.15.
...
**Set throughput degrades (-79% from 4→64 threads).** All writes on a node contend on the same SpinLock. At 32 threads/node, SpinLock contention dominates. This is the expected bottleneck for a single SpinLock per node; the documented upgrade path is per-thread PromoteBuf sharding (\S2.8.5) or striped SpinLocks.
...
**Comparison with single-lock baseline:** The baseline's concurrent Set with 4 threads is 2,205,136 ops/s (§5.18). Furrballs at 4 threads is 3,831,728 (1.74x advantage). At 8 threads, if the baseline scales poorly (single mutex, 8 threads contending), Furrballs' per-node sharding (4 threads/node × 2 nodes) should maintain or increase the advantage. A baseline thread-scaling test would quantify this, but is left as future work since the baseline uses `unordered_map` which is not thread-safe for concurrent writes.
...
6. **Set throughput degrades -79% from 4→64 threads** --- SpinLock bottleneck at 32 threads/node (documented upgrade path: per-thread PromoteBuf sharding)
...
#### Phase 2b: Eviction + Persistence
- [ ] Wire eviction callback to flush dirty values to RocksDB
