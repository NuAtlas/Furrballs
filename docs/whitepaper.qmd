...
## Status
**v1.27.0** --- PRELIMINARY multi-baseline NUMA comparison. Corrected capacity ratio (62.5×, not 32×). Corrected all table numbers to match raw benchmark data. Added 16pg and 256pg capacity sweep. Documented RR partitioned outlier (CV=65%, N=3 insufficient). Disclosed trace replay uses 200K/1M entries. Added per-node asymmetry note and TL-vs-RR decomposition as publication requirement. Migration remains untested.
...
| System | Effective Capacity | Concurrency | Eviction Policy | Pool Size |
|--------|----:|---|----:|----:|
| FurrBall TL | 512 KB (2 × 256 KB pages) | 1 SpinLock per node | ARC (FlatList) | — |
| FurrBall RR | 512 KB (same) | 1 SpinLock per node | ARC (FlatList) | — |
| TBB | Unbounded | Per-bucket | None (no eviction) | — |
| CacheLib | 32 MB (clamped minimum) | Per-bucket striped | LRU | ~16 MB pool |
