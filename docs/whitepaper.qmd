...
subtitle: "Technical Report v1.9.0"
...
## Status
v1.9.0 --- ReARC: Reduction-based Adaptive Replacement Cache --- unified 2D state space (S_local, S_remote) producing dual action scores (Evict, Migrate) via precomputed SIMD lookup tables
...
ReARC is a new design direction (Direction A from §3.28). Direction B (ConcurrentARC for eviction + separate migration policy) remains documented as an alternative. The two directions are not mutually exclusive — ReARC could replace ConcurrentARC's key-level policy while ConcurrentARC's CMap backing remains unchanged (CMap provides concurrent storage, ReARC provides the policy).

| Component | Direction A (ReARC) | Direction B (complement ARC) |
|---|---|---|
| CMap | Unchanged | Unchanged |
| ConcurrentARC lists (t1/t2/b1/b2) | Replaced by ReARC's (S_local, S_remote) | Retained for eviction |
...
5. **Interaction with page-level ARC:** If ReARC replaces key-level ARC entirely, does page-level ARC still exist? ReARC's Evict score per key could be aggregated per page to drive page-level eviction, eliminating the need for a separate page-level ARC.
