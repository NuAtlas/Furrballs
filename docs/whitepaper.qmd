...
![YCSB p50 GET latency across systems for workloads A, B, and C (64B values, 2T, c6a.metal). FurrBallTL achieves 80ns across all workloads --- 22x faster than CacheLib's 1800ns on YCSB-A.](paper/figures/ycsb_latency_64b.png)
...
![FurrBallTL vs CacheLib head-to-head across YCSB workloads A/B/C (64B, 2T). Left: p50 GET latency. Right: throughput.](paper/figures/ycsb_tl_vs_cachelib.png)

**Value-size sweep (YCSB-B, 95R/5W, 2T):**

![p50 GET latency (left axis) and hit rate (log scale) across value sizes for YCSB-B, 2T. At 256B, FurrBallTL maintains 100% hit rate while CacheLib drops to 95%. At 1024B (100MB working set vs 32MB cache), all bounded caches show ~94% hit rate but FurrBallTL achieves it at 80ns vs CacheLib's 370ns.](paper/figures/ycsb_latency_sweep.png)

| Value size | Working set | FurrBallTL p50g | CacheLib p50g | TL hit% | CL hit% |
|---|---|---|---|---|---|
...
![Thread scaling before (left) and after (right) per-node stat sharding fix. Before: FurrBallTL degrades 6.6x from 2T to 4T. After: FurrBallTL scales flat at ~70ns across all thread counts, matching TBB's scaling behavior.](paper/figures/thread_scaling_fix.png)
...
5. **4T TL anomaly resolved** --- root cause: `alignas(64)` statistics atomics on heap object caused cross-socket cache-line bouncing. Fix: per-node stat sharding. 4T p50 GET: 587ns -> 71ns (-88%)
...
- [x] Statistics with cache-line-aligned atomics (per-node sharding v1.33.0)
- [x] Benchmark harness
