...
subtitle: "Technical Report v1.25.0"
...
## Status
**v1.28.0** --- PRELIMINARY multi-baseline NUMA comparison. Added NUMABench methodology section (adapter architecture, thread placement, workload specs, latency measurement, per-system memory allocation). Documented CacheLib's accidental NUMA interleaving (shm allocated from unpinned main thread, no setMemBind configured). All prior v1.27.0 corrections remain.
...
### 5.24 Preliminary Multi-Baseline NUMA Comparison (c6i.metal)

> **Status: Preliminary.** Single run, no capacity normalization, no confidence intervals. These results are directionally informative but not publishable as-is. Capacity normalization and multi-run confidence intervals are required before inclusion in the evaluation narrative.

**Platform:** AWS c6i.metal (spot), Intel Xeon Platinum 8375C @ 3.5GHz, 2 sockets, 64 cores/socket, 128 vCPUs (HT), ~257 GB RAM, NUMA distance matrix 10/20. Ubuntu 24.04, GCC 14.2.0, Release build. 3 repetitions per benchmark.

#### Methodology

**Benchmark architecture.** NUMABench (`Benchmark/NUMABench.cpp`) uses a template-parameterized adapter pattern. Each system (FurrBall TL, FurrBall RR, TBB, CacheLib) implements a four-method interface: `create(nodeCount, pagesPerNode)`, `get(key, buf, bufSize, outSize) → bool`, `put(key, data, size)`, `destroy()`. No virtual dispatch — each adapter is a compile-time template parameter. Google Benchmark manages iteration counting and timing; the benchmark code runs the full workload inside each iteration and aggregates per-thread latency vectors.

**Thread placement.** Each worker thread is pinned to a NUMA node via `Numatic::PinCurrentThreadToNode(n)`, which calls `numa_run_on_node(n)` + `sched_setaffinity`. With 2 threads, thread 0 is pinned to node 0 and thread 1 to node 1. The main benchmark thread (which calls `sys.create()` and `sys.destroy()`) is not pinned.

**Workloads.** Each thread processes 100,000 operations (`OPS_PER_THREAD = 100000`). Value size is fixed at 64 bytes for all workloads and all systems.

- *Partitioned (Zipfian, theta=0.99, universe=100K):* Each thread accesses a disjoint key range (`[T × 50000, (T+1) × 50000)`). On a miss, the key is inserted with a 64-byte value. This measures per-thread-local access latency under NUMA-aware vs NUMA-blind routing.
- *Shared (Zipfian, theta=0.99, universe=100K):* Both threads access the same 100K-key space. This measures contention behavior under concurrent access to the same keys.
- *Trace replay (Twitter cluster 52):* The first 200K entries of a 1M-entry trace are loaded. Thread 0 processes even-indexed entries, thread 1 processes odd-indexed entries, for a total of 200K operations across both threads. Keys from the trace are used directly as string keys; the trace's original sizes are ignored and all values are 64 bytes.

**Latency measurement.** Each individual `get()` and `put()` is timed with `std::chrono::high_resolution_clock`. Per-thread latency vectors are sorted at the end of each iteration to compute p50 and p99. The reported p50 is the overall p50 across all threads (not the mean of per-thread p50s). Per-node p50s are also reported for local-vs-remote analysis.

**Systems configuration.**

| System | Effective Capacity | Concurrency | Eviction Policy | Memory Allocation | NUMA Awareness |
|--------|----:|---|----:|---|---|
| FurrBall TL | 512 KB (2 × 256 KB) | 1 SpinLock/node | ARC (FlatList) | `numa_alloc_local()` per node | Explicit: pages on local node |
| FurrBall RR | 512 KB (same) | 1 SpinLock/node | ARC (FlatList) | `numa_alloc_local()` per node | Explicit: pages on local node |
| TBB | Unbounded | Per-bucket | None | `operator new` | None (kernel default) |
| CacheLib | 32 MB (clamped min) | Per-bucket striped | LRU | SysV shm from main thread | **None configured** (see note) |

**CacheLib NUMA note:** CacheLib supports NUMA-aware allocation via `MemoryTierCacheConfig::setMemBind()`, which calls `mbind()` on its shared memory segments. Our adapter does not configure this — it uses CacheLib's default constructor with no `setMemBind()`. The `create()` call runs on the unpinned main benchmark thread. CacheLib allocates via SysV shared memory (`shmget`/`shmat`), and the kernel places pages according to the calling thread's NUMA policy. Since the main thread is unpinned, the kernel's default policy applies (typically interleaved across both nodes). This means CacheLib's 32MB slab pool is almost certainly **interleaved** across NUMA nodes — approximately half local, half remote from any single thread's perspective. This is accidental, not designed: CacheLib does not know which thread will access which data. This gives CacheLib a ~50% NUMA penalty on average for every access, while FurrBall TL pays ~0% for local hits. This asymmetry benefits FurrBall TL in the partitioned comparison and should be controlled in future runs (either by pinning CacheLib's allocation thread, or by configuring `setMemBind` per-pool).

**Known handicaps for FurrBall:**

1. **62.5× less capacity.** FurrBall at 64 pages/node = 512 KB total (2 × 256 KB). CacheLib's allocator clamps to a 32 MB minimum (64 × 4 KB → max(256 KB, 32 MB) = 32 MB), giving it a 16 MB allocation pool. The ratio is 32 MB / 512 KB = 62.5×. TBB has unbounded capacity (no eviction at all). This directly affects hit rate on trace replay (the only workload where capacity matters).
2. **Single SpinLock per node.** CacheLib uses per-bucket striped locking (many fine-grained locks). TBB uses per-bucket fine-grained locking. FurrBall has one SpinLock guarding the entire node's ConcurrentARC. At 2 threads (1 per node), contention is low, so this handicap is mild here but scales poorly (see §5.19 for the 4→64 thread scaling data).
3. **ARC vs no eviction.** FurrBall does real ARC eviction (adaptive, p-adaptation between recency and frequency). CacheLib uses LRU (simpler, less adaptive). TBB does not evict at all. Under capacity pressure, ARC should produce higher hit rates than LRU, but we cannot observe this because the capacity gap dominates.

**Workloads:**

- **Partitioned (Zipfian, theta=0.99):** Each thread accesses its own key space. All systems show ~95.7% hit rate because the Zipfian hot set fits everywhere. This measures raw access latency under NUMA-aware vs NUMA-blind routing.
- **Shared (Zipfian, theta=0.99):** Both threads access the same key space. Measures contention behavior under NUMA-aware vs NUMA-blind routing. Not a NUMA topology test; all systems converge to similar latency under contention.
- **Trace replay (Twitter cluster 52, 200K of 1M entries):** Trace replay with realistic access patterns. Each thread processes 100K operations from the first 200K entries of the 1M-entry trace (interleaved across threads). Hit rate reflects both policy quality and effective capacity. This is where the capacity handicap dominates.

#### Results: Partitioned (2 threads, 64 pages)

| System | p50 Get (ns) | p99 Get (ns) | p50 Set (ns) | Hit Rate |
|--------|---:|---:|---:|---:|
| FurrBall TL | 210 | 518 | 434 | 95.7% |
| FurrBall RR | 210 (332 mean) | 1,003 | 1,744 | 95.7% |
| TBB | 58 | 563 | 292 | 95.7% |
| CacheLib | 248 | 890 | 1,264 | 95.7% |

All values are 3-iteration means from Google Benchmark, except FurrBall RR p50 which is annotated with mean because one of three iterations produced a p50 of 582ns (CV=65%, see outlier note below).

The NUMA signal is isolated by comparing FurrBall TL vs RR (same system, same capacity, only routing differs). On two of three iterations, RR produces p50 ≈ 208ns — nearly identical to TL. One outlier iteration (582ns) inflates the mean to 332ns. This suggests the NUMA penalty for round-robin routing is small when both threads happen to land keys near their local node, but can spike when the hash distribution aligns poorly. The clean TL-vs-RR comparison requires more repetitions to produce a stable estimate.

FurrBall TL is **15% faster than CacheLib on local gets** (210ns vs 248ns) despite having 62.5× less capacity and doing real ARC eviction while CacheLib barely evicts. CacheLib's per-bucket striped locking provides no advantage at 1 thread per node. A 3--6% per-node asymmetry is visible in the raw data (node0 p50=219ns, node1 p50=206ns for FurrBall TL), likely reflecting minor NUMA distance asymmetry on the spot instance.

TBB's 58ns gets reflect its hash-table-only nature: no eviction overhead, no ARC metadata, no lock contention at 2 threads. This is the throughput ceiling for a system that doesn't manage eviction.

**Outlier note:** The RR partitioned p50 has CV=65% across 3 iterations (205ns, 210ns, 582ns). The 582ns iteration also shows elevated per-node p50 (node0=600ns, node1=568ns), suggesting OS scheduling noise or NUMA page migration during that iteration. With N=3, a single outlier dominates the mean. More repetitions are needed for a reliable RR-vs-TL comparison.

#### Results: Shared (2 threads, 64 pages)

| System | p50 Get (ns) | p99 Get (ns) | Hit Rate |
|--------|---:|---:|---:|
| FurrBall TL | 618 | 1,256 | 95.7% |
| FurrBall RR | 562 | 1,101 | 95.7% |
| TBB | 612 | 871 | 95.7% |
| CacheLib | 439 | 1,087 | 95.7% |

All systems converge to ~600ns under shared contention, confirming this is a contention test, not a NUMA topology test. CacheLib wins at 439ns due to its per-bucket striped locking, which provides finer-grained concurrency than FurrBall's single SpinLock per node. **This is a legitimate FurrBall weakness.** The documented upgrade path is striped SpinLocks or per-bucket ARC locks.

Notable: FurrBall RR (562ns) is slightly faster than FurrBall TL (618ns) under contention. This is because round-robin distributes lock acquisitions more evenly across nodes, reducing SpinLock contention per node. Thread-local routing concentrates all of thread 0's operations on node 0's SpinLock, creating a hotspot.

#### Results: Trace Replay (2 threads, 64 pages)

| System | p50 Get (ns) | p99 Get (ns) | Hit Rate |
|--------|---:|---:|---:|
| FurrBall TL | 343 | 1,013 | 76.5% |
| FurrBall RR | 355 | 1,016 | 76.5% |
| TBB | 297 | 902 | **80.3%** |
| CacheLib | 631 | 1,965 | **80.1%** |

This is where the capacity handicap dominates. The 3.8pp hit rate gap between FurrBall (76.5%) and CacheLib/TBB (~80%) is almost entirely due to capacity. CacheLib at 32MB barely evicts; TBB never evicts. FurrBall at 512KB does real ARC eviction under the Twitter trace. The hit rate difference reflects how many keys CacheLib/TBB can retain that FurrBall must evict.

CacheLib's 631ns p50 on trace replay is 83% slower than TBB's 297ns. This is because CacheLib pays LRU promotion + slab allocation overhead on every access, while TBB's concurrent_hash_map is a pure hash table. CacheLib's trace latency gap is a data structure cost, not a NUMA cost.

#### Synthesis

The three tests isolate different aspects of system performance:

1. **Partitioned** attempts to measure the raw NUMA topology signal. The intended TL-vs-RR comparison is confounded by a single outlier iteration (CV=65% for RR p50). On two stable iterations, TL and RR produce nearly identical p50 (~208ns), suggesting the cross-NUMA penalty for round-robin routing may be smaller than initially estimated. More repetitions are needed. FurrBall TL's 15% get-speed advantage over CacheLib (210ns vs 248ns) despite 62.5× less capacity remains the key finding: FurrBall's page arena + FlatList ARC has lower per-operation overhead than CacheLib's slab allocator.

2. **Shared** measures concurrency model quality. CacheLib's striped locking (439ns) beats FurrBall's single SpinLock (618ns) by 29%. This identifies the concurrency model as the next bottleneck to address.

3. **Trace replay** measures the combined effect of policy quality and capacity. The 3.8pp hit rate gap between FurrBall (76.5%) and CacheLib/TBB (~80%) is almost entirely a capacity artifact. Normalizing capacity would either close this gap (validating ARC's superiority over LRU) or widen it (if ARC's overhead negates the hit rate benefit). This test cannot distinguish the two without capacity normalization.

#### Additional Capacity Points (16 and 256 pages)

The benchmark also tested 16 pages/node (32 KB total for FurrBall) and 256 pages/node (2 MB total):

**Partitioned:**

| Pages/node | FurrBall TL | FurrBall RR | TBB | CacheLib |
|---:|---:|---:|---:|---:|
| 16 (32 KB) | 210 ns | 360 ns | 58 ns | 238 ns |
| 64 (512 KB) | 210 ns | 210 ns (outlier: 332 mean) | 58 ns | 248 ns |
| 256 (2 MB) | 217 ns | 334 ns | 58 ns | 237 ns |

FurrBall TL's partitioned p50 is stable at 210ns across all capacities, confirming the access path cost is independent of capacity (the Zipfian hot set fits everywhere). TBB is constant at 58ns. CacheLib varies between 237--248ns (slight slab allocation variation). FurrBall RR continues to show high variance at 16pg (360ns) and 256pg (334ns), suggesting the RR instability is not capacity-dependent.

**Shared:**

| Pages/node | FurrBall TL | FurrBall RR | TBB | CacheLib |
|---:|---:|---:|---:|---:|
| 16 (32 KB) | 614 ns | 462 ns | 607 ns | 460 ns |
| 64 (512 KB) | 618 ns | 562 ns | 612 ns | 439 ns |
| 256 (2 MB) | 636 ns | 595 ns | 593 ns | 382 ns |

CacheLib's shared p50 improves with more capacity (460→439→382ns), likely because larger slab pools reduce allocation contention. FurrBall TL's shared p50 is stable (614--636ns), confirming the SpinLock is the bottleneck regardless of capacity.

#### Why These Results Are Directionally Informative But Not Publishable

The directional conclusions are:

- FurrBall's access path is architecturally efficient (15% faster gets than CacheLib despite 62.5× less capacity)
- The concurrency model is the binding constraint for shared workloads (CacheLib 29% faster than FurrBall)
- The TL-vs-RR comparison is inconclusive with N=3 (outlier-dominated)
- The capacity handicap obscures the policy comparison

What's missing for publication:

- **More repetitions.** N=3 is insufficient. The RR partitioned test has CV=65% — a single outlier (582ns) makes the mean 59% higher than the other two iterations (205ns, 210ns). Minimum N=10, ideally N=30.
- **Capacity normalization.** Run CacheLib at 512KB total (same as FurrBall) to isolate policy quality from capacity. This requires removing the 32MB clamp in the CacheLib adapter.
- **TL-vs-RR decomposition.** The RR overhead includes double-lookup cost (probe local node, miss, probe remote) in addition to the cross-NUMA memory penalty. A "single-node FurrBall" config (forcing all keys to one node regardless of routing) would isolate the pure NUMA penalty from the double-lookup cost.
- **Thread scaling.** The 2-thread comparison doesn't exercise FurrBall's SpinLock bottleneck. A 4+ thread test would show where the concurrency model breaks.
- **Migration.** This is the other half of Furrballs' value proposition and is entirely untested.

## 6. Current Status

**Phase 1 is complete.** **Phase 2a is complete.** **Phase 2b data collected but evaluation deferred** (§5.20: page-level eviction smothers policy differentiation). **ARC-FLATLL2 optimized** (uint32_t indices + arena + no cached_: +87--172% over ARC, ranked #2 KPI). **ARC-REMARC Hybrid tested** (§5.23: negative finding, REMARC scoring does not subsume p-adaptation, complementary not substitutable). **History Atom experiment** (retracted; REMARC paper old §10-§11 removed, Findings 35--38 withdrawn): thorough negative finding documented. Post-publication audit (v0.13.0) revealed score degeneracy and sentinel bug that invalidated all reported HA hit rates. Genuine HA performance is LRU-equivalent (~25% Zipfian). See REMARC paper §10 for full analysis. PolicyBench includes 48+ variants with all sentinel bugs fixed.
