...
subtitle: "Technical Report v1.17.0"
...
## Status
v1.17.0 --- §5.22 ARC-FLATLL2 limits documented and tested (15/15 pass): uint32_t capacity bound (2.1B), per-list 2×cap occupancy, cap=1 edge case, pure scan, single-key, high churn, thread safety. --limits mode added to PolicyBench.
v1.16.0 --- §5.22 updated with ARC-FLATLL2 optimized variant: uint32_t indices (Slot 32→20 bytes), pre-allocated arena, removed redundant cached_ set. ARC-FLATLL2 beats ARC by +87--172% throughput, ranked #2 KPI (71.60) vs ARC #7 (59.78). Hit rates identical.
v1.15.0 --- §5.22 ARC-Flat: Data Structure Improvement to ARC. Three variants compared (ARC/ARC-FLAT/ARC-FLATLL), all produce identical hit rates. ARC-FLATLL (flat-linked-list) beats ARC-linked on every workload (+1.5% to +34%), ranked #3 overall vs ARC #6. Free-list eliminates all per-operation heap allocation. Implementation details, optimization opportunities, REMARC+ARC hybrid design outlined.
v1.14.0 --- §5.21 per-key eviction design plan: two-level REMARC architecture (KeyPolicy + PagePolicy), 2×2 factorial benchmark (ARC vs REMARC, 1-level vs 2-level), ARC data structure finding (linked list 2-3x slower than flat array, not fundamental to ARC algorithm), 3-way comparison plan (ARC-linked vs ARC-flat vs REMARC-flat). Roadmap updated: Phase 2c (per-key eviction), Phase 2d (two-level + optimization). §7 Risks updated with ARC double-edged sword analysis. REMARC-BLOOM noted as speed baseline.
v1.13.0 --- Phase 2b benchmark data collected on c6i.metal (commit 6e734ad). §5.20 added with full tables, DEFERRED status. Page-level eviction smothers policy differentiation: hit rates identical across all policies in all workloads, 2.6x latency gap is data structure artifact (ArcList vs TempCtrl layout), migration only fires on LOOP. Policy comparison deferred pending benchmark redesign to isolate data structure overhead from policy decisions.
v1.12.0 --- REMARC paper v0.8.0 with Finding 31 (QuotaFree: pure prediction insufficient without structural scan detection). Consistency pass on both papers (53 issues: heading numbers, cross-refs, stale data, variant counts, Furrballs refs removed from REMARC paper). Whitepaper updated: QuotaFree in taxonomy, Related Work "two-level ARC"→"REMARC unified policy", §2.1.1 future tense fixed.
...
The companion paper [Sphynx2025] provides the full evaluation, theory, and algorithmic details. Furrballs serves as the implementation platform and target for real-NUMA deployment.
...
| Sphynx (2025) [Sphynx2025] | General cache policy framework (REMARC) | Algorithm-level policy framework; no NUMA awareness, no systems implementation |
| Furrballs | Per-page placement + REMARC unified policy | Fine-grained, adaptive, page-level NUMA locality as policy input |

To our knowledge, no published system implements page-level cache eviction policy informed by NUMA placement and cross-node access frequency. NUMA-aware allocators (TCMalloc, jemalloc) handle placement but not eviction; NUMA-aware caches (CacheLib) use static sharding without adaptive per-page locality decisions. Rayhan & Aref's `move_pages2` syscall improves kernel-level page migration efficiency for DBMS workloads (up to 1.84x query throughput improvement) but does not address user-space cache placement policy---it is complementary to Furrballs' approach, which avoids runtime page migration entirely via track-and-defer eviction. Rambus's US11561834B2 patent describes an ML-based reinforcement learning system for adaptive metablock placement across NUMA tiers, operating as an external optimizer above the OS rather than as a user-space caching library with integrated NUMA-aware eviction. The companion REMARC paper [Sphynx2025] (DOI: 10.5281/zenodo.19794758) formalizes the policy framework and evaluates 24 variants across five workloads; Furrballs provides the systems platform for NUMA-aware deployment of that framework.
...
All previous evaluation (§5.2--§5.17) was conducted on a QEMU VM with 2 NUMA nodes and 4 vCPUs. QEMU does not simulate real cross-node memory latency (§5.9); the measured NUMA effects reflected scheduling artifacts rather than hardware memory access costs. This section presents results from real multi-socket hardware.
...
**A1: NUMA Cross-Node Overhead** (ARC, thread-local routing, 500 distinct keys, 10 iterations)
...
This is analogous to Memcached's model: slab = allocation unit (our page), item = eviction unit (our key), slab reclamation = lazy/when-empty (our Level 2 recycle). Furrballs adds REMARC scoring at both levels plus NUMA-aware placement.
...
#### Next Step: REMARC + ARC-FLATLL Hybrid

The flat-linked-list provides the data structure foundation. The next step is combining ARC's adaptive p-adjustment with REMARC's per-key desire scoring:

- **ARC provides**: p-adaptation (recency vs frequency balance), ghost lists (adaptive learning)
- **REMARC provides**: per-key TempCtrl scoring (2-atom desire composition), SIMD-scannable eviction selection
- **Hybrid**: ARC's p-adaptation decides which list to evict from (T1 or T2), REMARC's EvictScore selects which specific key within that list to evict (not always the LRU)

This would be a true two-level composition: ARC at the list level (architecture decisions), REMARC at the key level (scoring decisions). The flat-linked-list layout makes both levels efficient — ARC operations via index linking, REMARC scoring via contiguous TempCtrl array.

## 6. Current Status

**Phase 1 is complete.** **Phase 2a is complete.** **Phase 2b data collected but evaluation deferred** (§5.20: page-level eviction smothers policy differentiation). **ARC-FLATLL validated** (§5.22: flat-linked-list closes data structure gap). **ARC-FLATLL2 optimized** (uint32_t indices + arena + no cached_: +87--172% over ARC, ranked #2 KPI).
...
#### Phase 2c: Per-Key Eviction + Fair Policy Comparison
- [ ] Per-key eviction: tombstone + dead-bytes, no in-page reuse (simplest viable implementation)
- [ ] EvictOneKey: scan pages for min EvictScore key, remove from Page + KeyStore
- [ ] Page.h: dead-bytes tracking per page, page recycling when ActiveKeys == 0
- [ ] ARC per-key eviction: use ArcList position (tail of T2) for victim selection instead of page recycling
- [ ] 2x2 factorial benchmark: ARC-1L vs REMARC-1L (scoring quality), REMARC-1L vs REMARC-2L (architecture benefit)
- [ ] REMARC-BLOOM speed baseline: page-level only, Bloom filter membership, no per-key state
...
- [ ] REMARC + ARC-FLATLL hybrid: ARC p-adaptation (list level) + REMARC TempCtrl scoring (key level)
...
Phase 2b implements REMARC (Reduction-Modeled Adaptive Replacement Cache), replacing the planned two-level ARC hierarchy with a single unified policy. REMARC reduces three access dimensions (recency, frequency, locality) into a 2D state space (S_local, S_remote) stored per-key in TempCtrl bytes. Per-key Evict and Migrate scores are computed via precomputed 256-entry lookup tables scored with SSE2 SIMD arithmetic in O(1) per page. Per-key Evict scores aggregate to per-page eviction decisions, eliminating the need for a separate page-level ARC. Adaptation is outcome-based (eviction reversal and migration reversal rates adjust thresholds theta_e and theta_m), providing ARC's self-tuning principle without ghost lists. The probabilistic HotNode mechanism (biased sampling) provides migration targets with built-in anti-ping-pong protection. The three-mechanism model (key-level ARC, page-level ARC, cross-node migration policy) collapses to one: REMARC.

During Phase 2b development, REMARC evolved into a general policy framework formalized in a companion paper [Sphynx2025] (DOI: 10.5281/zenodo.19794758). The framework expresses cache policies as compositions `E = P(A1, ..., An)` where atoms extract access signals and projections map them to eviction decisions. Twenty-four variants were evaluated across five workloads (Zipfian, ScanResistant, Temporal, Looping, Uniform). Key results: (1) page-level eviction imposes a structural ~24.5% Zipfian ceiling absent per-key scoring, (2) ARC is a strict subset of the framework (setting demote threshold to minimum reproduces ARC exactly), and (3) AUG-ADAPT, a self-tuning variant combining heap-based eviction with ARC's quota structure via a feedback controller (population CV + prediction regret), achieves 99.33% hit rate on adversarial looping workloads while matching ARC on Zipfian/ScanRes/Temporal. The delivery--quota tension---where accurate eviction selection corrupts ARC's self-tuning---is resolved by the controller's conservative default (LRU) with workload-driven activation of heap eviction.
...
Phase 2b continues with RocksDB-backed page persistence and multi-node validation of AUG-ADAPT via desire-encoded migration. The broadcast-race dispatch model (Phase 4) provides O(N) aggregate work with O(1)-round parallel latency.
...
Furrballs/
  Furrballs/          # Core library (C++20, MIT license)
    include/
      ...
      Policy.h        # RemarcPolicy template, ArcPolicy, StandardRemarc alias
