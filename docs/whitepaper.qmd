...
subtitle: "Technical Report v1.16.0"
...
Three further optimizations were applied to ARC-FLATLL:

1. **`uint32_t` indices** instead of `size_t`: Slot shrinks from 32 to 20 bytes (8 key + 4 prev + 4 next + 1 active + 3 padding). 60% denser packing → better cache line utilization. For capacity ≤ 4B entries, `uint32_t` suffices.
2. **Pre-allocated arena**: Each list pre-allocates 2×capacity slots at construction (ARC allows any individual list to hold up to 2×cap entries in pathological cases). Free-list chains inactive slots. Zero malloc/free after construction.
3. **Removed redundant `cached_` set**: The original ARC implementation tracks cache membership in a separate `unordered_set<uint64_t>`. Since T1∪T2 IS the cache, membership is `t1_.contains(k) || t2_.contains(k)` — one redundant hash lookup eliminated per access.
...
The dominant optimization is the `uint32_t` compaction: reducing Slot from 32→20 bytes means 5 slots per 64-byte cache line instead of 2, a 2.5x improvement in cache line efficiency for list traversal operations (splice, erase). The pre-allocation eliminates vector growth overhead. Removing `cached_` saves one hash lookup per access (2 probes instead of 3 for cache membership check).
