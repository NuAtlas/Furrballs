...
subtitle: "Technical Report v0.14"
author: "The Sphynx"
date: last-modified
lang: en
format:
...
- [ ] Broadcast-race Get: parallel speculative probe to all node workers, first to find key writes result via CAS (resolves location opacity in O(1) instead of O(N) sequential scan). Partial analogy to distributed speculative reads (Dynamo). Only beneficial with non-uniform routing (thread-local) + NUMA-aware pinning + per-node memory allocation. **Invariant:** For resident keys, exactly one shard can return hit in the discovery race — the shard whose node holds the key's page. **Waste model:** `net_gain = latency_saved - (N-1) × loser_probe_cost`, where `latency_saved` is the sequential probe time eliminated and `loser_probe_cost` is the per-worker cost of an unsuccessful shard lookup. Analysis focuses on when `net_gain > 0`. **Fail-safe:** Disable race when location cache hit-rate is high (race unnecessary) or when `loser_probe_cost × (N-1)` exceeds sequential probe latency (race wasteful).
- [ ] Location cache (key→nodeID): after broadcast-race resolves a cold-miss location, cache the result. Subsequent Gets go directly to the correct node — no race needed. Minimal metadata (one integer per key). Eviction follows a score-based policy with tunable weights (recency-dominant by default, adjustable for empirical testing). This is the lightest form of cross-node key synchronization — cache what you found, evict what you don't use.
