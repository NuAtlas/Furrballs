...
## Status
**v1.29.0** --- Corrected-capacity NUMA comparison at 1024B values. All systems now at exactly 32MB usable capacity (previous runs had CacheLib at ~16MB usable due to slab metadata overhead, and CacheLibNuma at 64MB due to a floor-bump bug). Added memory footprint tracking (footprint_mb counter). Added Cross-Node (CN) and Remote-Only adapters for NUMA penalty isolation. CacheLib pool size fixed: `addPool(targetBytes)` instead of `addPool(targetBytes/2)`. CacheLibNuma capacity split fixed: per-pool = target/nodeCount. Key finding: at 1024B values, TL routing gives 56ns p50_get vs CN's 191ns (3.4x from remote memory), confirmed with per-node latency data. FurrBall uses 32MB footprint for 32MB usable; CacheLib requires 64MB footprint (2x memory tax for slab metadata).
...
> **Status: Corrected capacity.** All systems at 32MB usable. Previous runs had unequal capacities (FurrBall 32MB, CacheLib ~16MB usable, CacheLibNuma 64MB due to floor-bump bug). Capacity alignment and footprint tracking added in commit `d6c66c4`. Remote-Only data pending (adapter added in `bfc937b`, awaiting EC2 run).
...
#### NUMA Penalty Decomposition

The three benchmark configurations form a gradient of remote access probability:

| Configuration | Local % | Remote % | p50 Get (ns) | Source |
|---|---:|---:|---:|---|
| TL (ThreadLocal routing) | 100% | 0% | **56** | ReadOnly data |
| CN (RoundRobin, 2 nodes) | ~50% | ~50% | **191** | ReadOnly data |
| Remote-Only (pending) | 0% | 100% | **~300** (estimated) | Adapter added, run pending |

The relationship is approximately linear: each 50pp increase in remote access probability adds ~135ns. This implies a pure-remote access costs ~300ns at 1024B, of which ~240ns is the NUMA remote memory penalty (300ns remote - 60ns local base) and ~60ns is the local access cost (CMap probe + 1KB memcpy). Pending Remote-Only data will confirm the 300ns estimate.

At 64B values, this penalty is invisible (memcpy is ~5ns for 64B, masked by lock overhead). At 1024B, the 1KB remote memcpy dominates, making topology-aware placement measurable and valuable.
...
1. **Topology-aware placement eliminates remote memory cost.** TL at 56ns vs CN at 191ns (3.4x). This is the thesis claim, supported by direct measurement.
...
1. **No 100% remote measurement yet.** CN gives a 50/50 blend. The Remote-Only adapter (pending) will give the pure remote number. The 300ns estimate is extrapolated, not measured.
2. **No throughput advantage at equal capacity.** Despite faster Gets, FurrBall's ops/s (2.7M) is half of CacheLib's (5.5M). The p99 eviction storms dominate throughput. Faster Gets do not compensate for slower Sets.
3. **TL vs SN confounds lock distribution with topology.** TL has 2 SpinLocks, SN has 1. The TL-vs-SN gap (79ns vs 156ns) is lock distribution, not NUMA. Only TL-vs-CN isolates the NUMA penalty.
4. **Single data point per value size.** We only have 64B and 1024B. The crossover point where NUMA penalty exceeds lock overhead is unknown. A 256B sweep would identify it.
5. **c6i.metal only.** NUMA penalties are platform-specific (UPI bandwidth, memory controller contention). These numbers apply to Ice Lake Xeon with 2 sockets and may differ on AMD EPIC or ARM Neoverse.
...
| Version | Summary |
|---------|---------|
| v1.29.0 | Corrected-capacity NUMA comparison at 1024B. All systems at 32MB usable. CacheLib pool fix (targetBytes not targetBytes/2). CacheLibNuma capacity split fix. Memory footprint tracking. CN and Remote-Only adapters. NUMA signal: TL 56ns vs CN 191ns (3.4x). FurrBall 100% memory efficiency vs CacheLib 50%. |
