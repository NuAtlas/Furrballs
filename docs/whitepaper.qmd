...
subtitle: "Technical Report v1.6"
...
## Status
v1.6 --- Phase 2a complete: CMap + ConcurrentARC + ArcList + PromoteBuf implemented, benchmarked, and evaluated
...
The five-step ablation study (§5.10, §5.17) isolates the contribution of each architectural decision across both phases. The Phase 2a ablation reveals that the `thread_local` cache fundamentally changed the routing dynamics: thread-local routing is now *faster* than round-robin (ablation D < C), unlike Phase 1 where the `GetCurrentNode()` syscall made it 34% slower. This has a methodological implication: Phase 1's routing overhead findings (26--41% improvement from thread-local routing, ablation C→D +34% Set increase) are **environment-specific** rather than architectural. They reflect QEMU's VM exit cost on `sched_getcpu()`, not an inherent property of NUMA-aware routing. On real hardware where `GetCurrentNode()` costs ~1--10ns via VDSO, the Phase 1 routing story would resemble Phase 2a's. The NUMA signal remains invisible in QEMU across both phases, reinforcing the need for real hardware validation.

Phase 2b will wire the eviction callback to flush dirty values to RocksDB, completing the L1 key-level ARC. The broadcast-race dispatch model (Phase 4) provides O(N) aggregate work with O(1)-round parallel latency. QEMU does not simulate real cross-node memory latency, so results validate the methodology and architecture but not the performance claims. Real hardware validation remains the critical step before publication.
