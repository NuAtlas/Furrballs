...
**Step 6: SIMD lookup tables.** Storing `S_local[4bit]` and `S_remote[4bit]` in one TempCtrl byte per key, the byte value `(S_remote << 4 | S_local)` is a direct index into a 256-entry precomputed lookup table. SIMD `pshufb` looks up 16 Evict/Migrate scores in a single instruction. Scanning a page of ~64 keys becomes 4 batches × ~6 SIMD instructions — effectively O(1) per page. The lookup tables are static (computed from the decision surface formula); only the thresholds (θ_e, θ_m) change at runtime.
...
The update model is a Bayesian rate estimation with exponential forgetting. S_local and S_remote represent smoothed estimates of the local and remote access rates. Observing an access of one type is evidence *against* the other type occurring in the same time window — so the non-observed rate decays. This is the principled justification for mutual erosion: if a remote access occurs, our estimate of the local access rate should decrease, because we occupied a time slot where a local access could have happened but didn't.

```
On local Get():
  S_local  += α_l × (15 - S_local)        // boost local rate estimate toward max
  S_remote *= (1 - α_r)                     // decay remote rate (observed local = evidence against remote)

On remote Get() from node N:
  S_remote += α_r × (15 - S_remote)        // boost remote rate estimate toward max
  S_local  *= (1 - α_l)                     // decay local rate (observed remote = evidence against local)
  with probability p(S_remote): HotNode = N  // probabilistic migration target

Time decay (per scan cycle):
  S_local  *= λ                             // both drift toward 0 (stale signals decay)
  S_remote *= λ
```
...
__m128i e  = _mm_shuffle_epi8(E_table_vec, tc);    // pshufb: 16 lookups in 1 instruction
__m128i m  = _mm_shuffle_epi8(M_table_vec, tc);
...
| SIMD scan | Precomputed lookup tables (pshufb) | Flag scanning (cmpeq) |
...
__m128i tc = _mm_loadu_si128(page.TempCtrl + batch);
__m128i e  = _mm_shuffle_epi8(E_table_vec, tc);    // 16 Evict scores
__m128i m  = _mm_shuffle_epi8(M_table_vec, tc);    // 16 Migrate scores
...
1. **4-bit vs 8-bit encoding:** 4 bits per score (16 levels) is SIMD-optimal (1 byte per key, pshufb) but coarse. 8 bits (256 levels) requires 2 bytes per key and SSE/AVX for arithmetic, but provides better precision for EMA smoothing. Empirical comparison needed.
