...
## Status
**v1.25.0** --- STRATEGIC CLOSURE. REMARC framework evaluated and retired. Comprehensive tiered placement simulation (K=2, K=3, feedback dimensionality) proves REMARC adds zero value over simple baselines (see REMARC paper §13). Production path switched from RemarcPolicy to SimpleMigratePolicy: ARC eviction + EMA-based migration counting. REMARC code retained for documentary purposes. AUG-ADAPT retained as historical variant. New SimpleMigratePolicy in Policy.h — compile-time template, no SIMD, no lookup tables, no RemarcConfig dependency.
...
A companion paper [Sphynx2025] (DOI: 10.5281/zenodo.19794758) documents the REMARC (Reduction-Modeled Adaptive Replacement Cache) framework that was originally developed for this system. After extensive evaluation --- including single-node eviction (score degeneracy, §12), multi-node placement simulation (K=2, K=3, predictive strategies, feedback dimensionality, §13), and theoretical analysis (cold-entry information hole, feedback SNR) --- REMARC was found to add zero value over simple EMA counting. The framework is retained as a thorough negative finding. The system now uses SimpleMigratePolicy as the production path and ARC for eviction.
...
Phase 2b initially implemented REMARC (Reduction-Modeled Adaptive Replacement Cache) as a unified eviction/migration policy, but extensive evaluation proved that the REMARC framework adds zero value over simple baselines. The companion REMARC paper [Sphynx2025] (v0.14.0, DOI: 10.5281/zenodo.19794758) documents the full evaluation: score degeneracy (§12), tiered placement simulation across K=2 and K=3 nodes with 10+ strategies (§13), and theoretical analysis of feedback signal quality (§13.5). Key findings: (1) the REMARC formula contributes zero eviction improvement over LRU, (2) all REMARC migration variants are equivalent to simple EMA counting, (3) feedback for placement decisions has SNR ≈ 0 (migration success rate ~50% regardless of strategy), and (4) "do nothing" (static placement) beats all adaptive strategies on half the tested workloads. The production path now uses ARC for eviction (proven, well-understood) and SimpleMigratePolicy for migration (two 4-bit EMA counters per key, no lookup tables, no SIMD).
...
| Version | Summary |
|---------|---------|
| v1.25.0 | Strategic closure. REMARC retired. Production path: ARC eviction + SimpleMigratePolicy. REMARC paper v0.14.0: comprehensive tiered placement evaluation (§13), feedback SNR analysis, REM-T algebra retired. |
