...
subtitle: "Technical Report v1.23.0"
...
## Status
**v1.23.0** --- Projection topology and state space dimensionality probes (REMARC paper §11, Findings 39-43). 22 probe variants. Finding 39 proves colinear signals add no value (dimensional independence). Finding 40: gap variance is a genuinely independent dimension (+2.92pp ScanRes). Finding 41: log-compressed projection closes ScanRes gap (+5.60pp). Finding 42: HA-LOGLIN-VAR achieves KPI 76.03, ranked #1 overall. REMARC paper v0.12.0. PolicyBench now includes 46 variants. `--policies` flag added for selective benchmarking.
...
| Version | Summary |
|---------|---------|
| v1.23.0 | Projection topology & state-space dimensionality (REMARC paper §11, Findings 39--43). 22 probe variants. HA-LOGLIN-VAR KPI 76.03 (#1). Log-projection +5.60pp ScanRes, gap variance +2.92pp. Colinearity proof: 4 signals zero change. REMARC paper v0.12.0. PolicyBench 45 variants, `--policies` flag. |
| v1.22.0 | History Atom experiment (REMARC paper §10, Findings 35--38). HA5: 57.5% Zipfian, 83.7% ScanRes, 90.7% Temporal, 96.1% Looping. REMARC paper v0.11.0. PolicyBench 36 variants. |
