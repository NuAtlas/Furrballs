...
subtitle: "Technical Report v1.32.0"
...
Benchmarks run on three platforms:
...
- **AWS EC2 metal (real hardware):** c6i.metal (Intel Ice Lake, 2 NUMA nodes, §5.18) and c6a.metal (AMD Milan, 4 NUMA nodes, §5.26)
...
4T TL per-node data: node0=660ns, node1=660ns, node2=660ns, node3=660ns --- all identical. Each thread has its own CMap, SpinLock, and maintenance thread, yet GET latency increases 6.6x from the 2T baseline (100ns). TBB remains flat at 40ns across all thread counts, ruling out system-level interference. The degradation is specific to FurrBall and likely stems from shared-global-state contention in the 4-instance configuration (4 maintenance threads, 4 CreateBall/destroy cycles per benchmark iteration, or memory allocation pressure from 4 concurrent `numa_alloc_onnode` pools). This anomaly is not present on c6i.metal (2 nodes), suggesting it scales with the number of NUMA domains. **Root cause investigation deferred** --- the 2T comparison is the fair head-to-head with CacheLib (2 sockets, 2 threads).
...
1. **4T anomaly unexplained.** TL at 4 threads (1 per node) shows 660ns GET, 6.6x worse than 2T (100ns). All 4 nodes show identical latency, ruling out NUMA as the cause. TBB scales flat, ruling out system interference. Root cause is likely FurrBall-specific shared state. The 2T head-to-head remains the fair comparison.
...
Real hardware (c6a.metal, AMD EPYC, 4 NUMA nodes) validated (§5.26):
1. **Cross-platform: topology-aware benefit confirmed on AMD** --- FurrBallTL 1.5x faster GET than CacheLib at 64B, 9.5x faster at 128MB working-set
2. **Vendor-independent: CacheLibNuma = CacheLib on AMD** --- per-pool routing without mbind is ineffective on both platforms
3. **AMD SpinLock sensitivity amplifies lock distribution** --- TL vs SN 3.4x on Milan vs 2.0x on Ice Lake
4. **CacheLib 50% memory tax confirmed on AMD** --- structural, not vendor-specific
5. **4T TL anomaly identified** --- 660ns GET at 4T vs 100ns at 2T, all 4 nodes identical, root cause deferred
...
- **4T TL anomaly on c6a.metal (4 NUMA nodes): GET degrades 6.6x from 2T to 4T (100ns to 660ns).** All 4 nodes show identical latency (660ns). TBB scales flat at 40ns. The degradation is FurrBall-specific and likely stems from shared-global-state contention scaling with NUMA domain count. Root cause investigation deferred --- the 2T head-to-head is the fair comparison with CacheLib (§5.26).
...
- [ ] Re-run Phase 2b workloads with per-key eviction, expect hit rate divergence
 - [x] Multi-platform validation (AMD EPYC Milan c6a.metal, §5.26)
