...
subtitle: "Technical Report v1.15.0"
...
**Phase 1 is complete.** **Phase 2a is complete.** **Phase 2b data collected but evaluation deferred** (§5.20: page-level eviction smothers policy differentiation). **ARC-FLATLL validated** (§5.22: flat-linked-list closes data structure gap, +1.5--34% throughput over ARC-linked, identical hit rates).
...
- **ARC's linked list is a performance bottleneck independent of policy choice.** ~~The 2.6x latency gap between ArcList (pointer-chasing doubly-linked list) and TempCtrl (contiguous array) is not fundamental to the ARC algorithm. A flat array with clock hand satisfies ARC's O(1) promote/evict/lookup requirements with better cache behavior. This is a publishable finding: ARC-flat would close the data structure gap while keeping ARC's adaptive p-adjustment. The policy comparison (ARC vs REMARC) should be conducted with equalized data structure layouts (§5.21: 3-way comparison ARC-linked vs ARC-flat vs REMARC-flat).~~ **Validated in §5.22.** ARC-FLATLL (flat-linked-list) eliminates heap allocation, beats ARC-linked by +1.5--34% throughput, produces identical hit rates. The data structure gap is closed. Next: REMARC + ARC-FLATLL hybrid (ARC p-adaptation + REMARC per-key scoring).
