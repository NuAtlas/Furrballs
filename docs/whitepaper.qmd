...
subtitle: "Technical Report v1.0"
author: "The Sphynx"
date: last-modified
lang: en
format:
...
Per-node `shared_mutex` serializes writes (map insert/update + page bump). Reads are lock-free via `SeqLock<KeyMeta>`: `Get()` iterates per-node shards, calls `SeqLock::Read()` on matching entries, and copies data without taking any lock. The SeqLock sequence counter provides deterministic consistency---if a writer is active (odd sequence), the reader retries. Statistics use `alignas(64)` on all atomic fields to prevent false sharing.
...
| Concurrent read throughput | Negligible (1.2x advantage from lock partitioning, offset on real hardware by NUMA latency) | Per-node shards reduce map size, improve cache locality |
...
The broadcast-race Get replaces the O(N) sequential shard scan with a parallel scatter-gather that resolves key location in O(1) expected time on a cold miss.
